...
rem Variant: LOVEDHEART AIMER-pruned 448-expert NVFP4+FP8_PB_WO
rem          (deployed via probe/lovedheart/deploy.sh; PLE streams
rem          from the vendor bin through our ssd-stream.json)
...
rem PORT   : host port 18082 (container port 8000)  [lovedheart profile]
...
if not defined POOL set /a "POOL= 2 * %CTX%"
...
rem lovedheart note: the pruned checkpoint ships MTP tensors inside the main
rem safetensors (no mtp/ dir), so NEXTN draft path is unavailable until we
rem synthesize mtp/ from those tensors. First validation runs SPEC=off.
set "SPEC=on"
set "DRAFT8=on"
...
echo   test request  : curl.exe http://127.0.0.1:%PORT%/v1/chat/completions -H "Content-Type: application/json" -d "{\"model":"%SERVED%\",\"messages\":[{\"role\":\"user\",\"content\":\"What is the capital of France? Answer in one word.\"}],\"max_tokens\":512}"
