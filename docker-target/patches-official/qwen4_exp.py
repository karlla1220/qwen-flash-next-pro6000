...
class Qwen4ExpNGramEmbedding(nn.Module):
    ...
    def __init__(
        ...
        # VocabParallelEmbedding's create_weights() allocates its initial
        # tensor with no device= kwarg, so during model construction (which
        # runs inside DefaultModelLoader's `with torch.device("cuda"):`) it
        # lands on GPU by ambient default-device context -- a transient
        # full-size (~48-95GB) GPU allocation, freed moments later once
        # Qwen4ExpPinnedHostEmbedding copies it to pinned host RAM below.
        # That transient spike alone can OOM hybrid state-cache sizing on a
        # near-full card when the table is BF16 (~95GB). When PLE is
        # offloaded this table's final home is pinned host RAM regardless,
        # so force the allocation onto CPU from the start.
        embedding_device_ctx = (
            torch.device("cpu") if config.ple_offload_embedding else nullcontext()
        )
        with embedding_device_ctx:
