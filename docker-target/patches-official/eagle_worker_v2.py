...
class EagleDraftWorker(EagleDraftWorkerBase):
    def __init__(
        ...
        # Backport of sgl-project/sglang#32468 (still open upstream as of
        # 593134d17): share the target's embed/lm_head here rather than in
        # alloc_memory_pool(). The scheduler runs init_target_memory_pool()
        # (which profiles free memory to size the KV cache) BEFORE
        # draft_worker.alloc_memory_pool(), so with the stock order the
        # draft's own embed/lm_head copies are still resident when the KV
        # profile runs and are only freed into an already-fixed budget.
        # Cost on this profile: ~1.3 GB with --speculative-draft-model-
        # quantization fp8 (draft-owned embed+head, vocab 248320 x hidden
        # 2560), ~2.5 GB with a bf16 draft.
        # Ported from docker-target/patches/eagle_worker_v2.py (SSD-stream
        # image) onto this image's stock file; stock drifted (get_spec()/
        # get_parallel() refactor, unwrap_lora_layer, draft_pp_context), so
        # only these 3 lines are re-applied, never the old file wholesale.
        self.init_token_map()
        self.init_lm_head()
        # set_embed_and_head only drops the Parameter refs; without an
        # explicit release the blocks stay in the caching allocator and
        # still count as used during KV sizing.
